...
.EXAMPLE
Enumarate\enumAzResourcesCsv.ps1
#>

$workingDir = '.\.assets\export\Resources'
...
foreach ($sub in $subs) {
    ...
    Get-AzResource | Select-Object -Property ResourceName `
        ...
        , @{Name = 'ConsumedService'; Expression = {($_.ResourceType -split('/'))[0] -replace('microsoft.', '')}} `
    ...
    } | Export-Csv -Path "$workingDir\enumAzResources_($($sub.Name)).csv" -NoTypeInformation -Encoding utf8
}
